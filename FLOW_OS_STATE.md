...
- **RESOLVED (2026-05-15, Slice 3c.1 round-2 review) — `awk -i inplace` followup (and broader `awk`/`sed` surface) closed by dropping both verbs from the allowlist.** Round-2 adversarial review surfaced 2 CRITICAL + 2 HIGH bypasses (awk BEGIN{system(...)}, sed -e `e` shell-escape, sed `r`/`w`/`R`/`W` file I/O, /tmp/.. path-traversal). Decision per Tyson: drop awk + sed from `SINGLE_VERBS` entirely rather than chase enumerated flag/body bans. Resolved in Slice 3c.1 round-2 remediation commit (this PR). See `CHARLIE_OVERHAUL.md` Slice 3c.1 scope amendment and Slice 3d planned redesign.
